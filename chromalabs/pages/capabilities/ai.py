...
def ai() -> rx.Component:
    return base_layout(
        rx.box(
            rx.vstack(
                rx.text("Spatial Weight Mapping", font_family="JetBrains Mono, monospace", font_size="1rem", color="#D4AF37", margin_bottom="4", letter_spacing="0.1em"),
                rx.text("ARTIFICIAL INTELLIGENCE", font_size=["3rem", "4rem", "5rem"], font_weight="800", line_height="1.1", color="white", letter_spacing="-0.03em", margin_bottom="8", text_align="center"),
                rx.text(
                ...
                rx.grid(
                    ...
                    rx.box(
                        ...
                        rx.text("By eliminating the fetch-execute cycle, the model itself becomes the hardware, unlocking a new paradigm of autonomous capabilities.", color="slate.10", font_size="1.1rem", line_height="1.6"),
                        padding="40px", background="rgba(255, 255, 255, 0.02)", border="1px solid rgba(255, 255, 255, 0.05)", border_radius="8px"
                    ),
                    rx.box(
                        rx.icon(tag="cpu", color="#D4AF37", size=32, margin_bottom="6"),
                        rx.text("Sparse Network Mastery", font_size="1.5rem", font_weight="bold", color="white", margin_bottom="4"),
                        rx.text("Architectures natively designed for extreme sparsity, maximizing computational efficiency without synthetic pruning penalties.", color="slate.10", font_size="1.1rem", line_height="1.6"),
                        padding="40px", background="rgba(255, 255, 255, 0.02)", border="1px solid rgba(255, 255, 255, 0.05)", border_radius="8px"
                    ),
                    rx.box(
                        rx.icon(tag="activity", color="#D4AF37", size=32, margin_bottom="6"),
                        rx.text("Zero-Latency Routing", font_size="1.5rem", font_weight="bold", color="white", margin_bottom="4"),
                        rx.text("Instantaneous data propagation through geometric planes ensures constant execution speeds irrespective of parameter scale.", color="slate.10", font_size="1.1rem", line_height="1.6"),
                        padding="40px", background="rgba(255, 255, 255, 0.02)", border="1px solid rgba(255, 255, 255, 0.05)", border_radius="8px"
                    ),
